...
const submitForm = async () =>{
    ...
    if(result){
            await axiosClient.put(`/categories/${route.params.id}`,{
...
onMounted(async() =>{
    ...
    if(restaurant.length > 0){
        const found = await (await axiosClient.get(`/categories?restaurantName=${route.params.name}&id=${route.params.id}`)).data
        if(found.length > 0){
            return
        }else{
            router.push('/Home')
